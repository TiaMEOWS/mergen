#!/usr/bin/env bash
# Shorten over-long directory SEGMENTS under .mergen/skill/** so paths fit
...
mapfile -d '' -t FILES < <(git ls-files -z -- '.mergen/skill/')
declare -A DSET=()
for f in "${FILES[@]}"; do
  d=${f%/*}
  while :; do
    case "$d" in .mergen/skill|.mergen|"" ) break;; esac
    DSET["$d"]=1
    d=${d%/*}
  done
done
