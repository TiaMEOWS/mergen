...
import { $ } from "bun"
import { createMergen } from "@mergen-io/sdk/v2"
import { parseArgs } from "util"
import { Script } from "@mergen-io/script"

export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://registry.npmjs.org/@mergen-io%2Fmergen").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/TiaMEOWS/mergen/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/mergen packages/sdk packages/plugin packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/mergen/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/mergen/")) areas.add("core")
...
async function summarizeCommit(
  mergen: Awaited<ReturnType<typeof createMergen>>,
  message: string,
): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await mergen.client.session.create()
  const result = await mergen.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "mergen", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], mergen: Awaited<ReturnType<typeof createMergen>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(mergen, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/TiaMEOWS/mergen/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  try {
    const mergen = await createMergen({ port: 0 })
    try {
      const lines = await generateChangelog(commits, mergen)
      notes.push(...lines)
      ...
      await mergen.server.close()
    }
  } catch (error) {
    console.log("Could not start mergen for changelog generation, using raw commits:", (error as Error).message)
    for (const commit of commits) {
      const attribution = commit.author && !Script.team.includes(commit.author) ? ` (@${commit.author})` : ""
      notes.push(`- ${commit.message}${attribution}`)
    }
