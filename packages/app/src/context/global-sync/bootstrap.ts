...
export async function bootstrapGlobal(input: {
  globalSDK: ReturnType<typeof createMergenClient>
    ...
    retry(() =>
      input.globalSDK.project.list().then((x) => {
        const projects = (x.data ?? [])
          .filter((p) => !!p?.id)
          .filter((p) => !!p.worktree && !p.worktree.includes("mergen-test"))
          .slice()
...
export async function bootstrapDirectory(input: {
  directory: string
  sdk: ReturnType<typeof createMergenClient>
  store: Store<State>
