...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("mergen://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("mergen://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("mergen://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("mergen://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("mergen://open-project")).toBeUndefined()
    expect(parseDeepLink("mergen://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "mergen://open-project?directory=/a",
      "mergen://other?directory=/b",
      "mergen://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MERGEN__: {
        deepLinks: ["mergen://open-project?directory=/a"],
      },
    } as unknown as Window & { __MERGEN__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["mergen://open-project?directory=/a"])
