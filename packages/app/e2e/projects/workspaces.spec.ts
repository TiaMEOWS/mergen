import { base64Decode } from "@mergen-io/util/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, withProject }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "mergen-e2e-project-nongit-"))
  ...
  try {
    await withProject(async () => {
      ...
      expect(path.basename(activeDir)).toContain("mergen-e2e-project-nongit-")
