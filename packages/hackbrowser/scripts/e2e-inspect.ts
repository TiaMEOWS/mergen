// E2E inspection: dump what Mergen captured + how proxy-agent interpreted it.
//
// Usage:
//   bun run scripts/e2e-inspect.ts --latest
//   bun run scripts/e2e-inspect.ts --session ses_XXX
//   bun run scripts/e2e-inspect.ts --latest --db /custom/path/mergen.db
//
// Reads the Mergen SQLite DB read-only and prints:
//   - session metadata
...
function defaultDbPath(): string {
  ...
  return path.join(base, "mergen", "mergen.db")
}
...
console.log(`Mergen E2E Inspection`)
