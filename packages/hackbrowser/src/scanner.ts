...
async function collectInteractiveElements(page: Page): Promise<BrowserElement[]> {
  return page.evaluate((): BrowserElement[] => {
    ...
    // hard browser limit) — including the Mergen panel's own closed root, so
    // the LLM never sees its own UI. We also skip any data-mergen-ui host
    ...
    function queryAllDeep(selector: string): Element[] {
      ...
      const walk = (root: Document | ShadowRoot) => {
        ...
        for (const host of root.querySelectorAll("*")) {
          if (host.closest("[data-mergen-ui]")) continue
    ...
    for (const el of queryAllDeep(INTERACTIVE_SELECTORS)) {
      // Skip anything inside the injected Mergen telemetry panel — LLM
      ...
      if (el.closest("[data-mergen-ui]")) continue
    ...
    for (const el of document.querySelectorAll<HTMLElement>("[aria-label]")) {
      if (el.closest("[data-mergen-ui]")) continue // defensive: never leak panel UI into info elements
      const tag = el.tagName.toLowerCase()
  ...
  try {
    await page.evaluate((max: number) => {
      ...
      for (const d of Array.from(document.querySelectorAll("details:not([open])"))) {
        if (budget <= 0) break
        if (d.closest("[data-mergen-ui]")) continue
      ...
      for (const c of Array.from(document.querySelectorAll('[aria-expanded="false"]'))) {
        if (budget <= 0) break
        if (c.closest("[data-mergen-ui]")) continue
