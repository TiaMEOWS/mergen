...
//
// Replaces the previous `mergen/util/log` import (Karar 2 — Dependency
// Inversion, INTEGRATION.md §5). Hackbrowser no longer depends on the
// mergen workspace package; this module provides the same API surface
// hackbrowser code already uses (Log.create, log.debug/info/warn/error).
//
// Default behavior: write to process.stderr.
// Mergen integration: call Log.setSink() to forward records into
// mergen's own logging pipeline (see hackbrowser-launcher.ts).
//
...
//
// Intentionally NOT carried over from mergen util/log:
//   - .tag(), .clone(), .time()  — unused in hackbrowser, YAGNI
//   - file logging                — hackbrowser always streamed to stderr
//   - Global.Path.log dependency  — coupled to mergen state
...
export namespace Log {
  ...
  /**
   * Replace the output transport. Default writes to stderr; mergen
   ...
   * intended pattern — same as mergen util/log.
   */
