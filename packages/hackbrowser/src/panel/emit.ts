 ...
 * eventSink callback (mergen-side telemetry) runs independently
...
/**
 * Module-level event sink. Mergen's hackbrowser-launcher registers
...
/**
 * Push an event to BOTH the injected browser panel AND the mergen
 * event sink (when registered). Two independent channels:
 *
 *   browser panel  → window.__csEvent (Shadow DOM live telemetry)
 *   eventSink      → mergen HackbrowserStatus (TUI sidebar)
 *
 ...
 * mergen sink might throw on Bus.publish. Neither should crash the
...
export async function csEmit(page: Page, event: CSEvent): Promise<void> {
  // Mergen sink — synchronous, runs first so the TUI sidebar gets
