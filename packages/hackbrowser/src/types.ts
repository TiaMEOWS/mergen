...
export interface CapturedRequest {
  ...
  raw: string
  // URL scheme captured at request time. Forwarded to Mergen so the
...
export interface IngestPayload {
  ...
  // older hackbrowser / Firefox extension builds; Mergen falls back
  ...
  // Mergen uses this with element_roles to determine available_roles per endpoint
  trigger_element?: string
  // Which roles can see the trigger element — derived from page_diff availability Map
  // Mergen proxy-analyzer uses this to set available_roles on web_function
...
/** Page-diff payload — sent once per page, element-level availability for Mergen */
...
export interface AgentConfig {
  ...
  mergen: {
    ...
    sessionID?: string
    credentialId?: string
    username?: string // --mergen-username (default: "mergen")
    password?: string // --mergen-password or MERGEN_SERVER_PASSWORD env var
  }
  auth: {
  ...
  outOfScope?: string[]
  // Network scope: hostnames whose requests get forwarded to Mergen.
  ...
  // Show browser window
  headless?: boolean
  // Dry-run mode: crawl without LLM calls, print captures to console instead of sending to Mergen
  dryRun?: boolean
  ...
  // Used by mergen launcher (Provider → opts.model → AgentConfig.model).
 ...
 * mergen launcher) decides exit code / user surface based on the
