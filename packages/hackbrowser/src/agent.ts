...
// verified end-to-end. NO effect unless MERGEN_HB_FAULT is set. Fires once
// mid-crawl (default page 3, override via MERGEN_HB_FAULT_AT) so you can
// confirm the crawl CONTINUES past the fault instead of the worker dying.
//   MERGEN_HB_FAULT=unhandled → an un-awaited Promise.reject (mimics #116)
//   MERGEN_HB_FAULT=uncaught  → a sync throw in a timer callback
let faultInjected = false
function maybeInjectFault(pagesExplored: number): void {
  const fault = process.env.MERGEN_HB_FAULT
  if (!fault || faultInjected) return
  if (pagesExplored < Number(process.env.MERGEN_HB_FAULT_AT ?? "3")) return
  faultInjected = true
  log.warn("injecting TEST fault (MERGEN_HB_FAULT)", { fault, pagesExplored })
  if (fault === "unhandled") {
...
async function runMultiCredential(config: AgentConfig, credentials: CredentialConfig[]): Promise<CrawlResult> {
  ...
  const serverUrl = config.mergen.serverUrl ?? "http://127.0.0.1:4096"
  ...
  // Single Mergen session for ALL credentials. Honor a host-provided
  // sessionID (mergen injects this when /hackbrowser slash or the
  ...
  let sessionId = config.mergen.sessionID ?? ""
  if (!dryRun && !sessionId) {
    const created = await initSession(serverUrl, targetUrl, undefined)
    if (!created) {
      await browser.close().catch(() => {})
      throw new Error(`failed to create Mergen session at ${serverUrl}`)
    }
    sessionId = created
  }
  ...
  for (const [credIndex, cred] of credentials.entries()) {
    ...
    // Register credential with Mergen (same as Firefox extension pattern)
  ...
  while (pageQueue.length > 0 && pagesExplored < maxPages) {
    ...
    // is the summarized form. Raw page-diff is NOT sent to Mergen (Decision 1).
    if (dryRun) {
      log.info("page-diff", {
        url: entry.url,
...
export async function run(config: AgentConfig): Promise<CrawlResult> {
  initAuth(config.mergen.username, config.mergen.password)
  ...
  const dryRun = config.dryRun ?? false
  const usageAcc: CrawlUsage = { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 }
  let credentialId = config.mergen.credentialId
  const panelOn = config.panel ?? true
  setPanelEnabled(panelOn)
  ...
  // Create Mergen session
  let sessionID = config.mergen.sessionID
  if (!dryRun && !sessionID) {
    const created = await initSession(serverUrl, targetUrl, credentialId)
    if (!created) {
      throw new Error(`failed to create Mergen session at ${serverUrl} — is Mergen running?`)
    }
    sessionID = created
  }
  ...
  // PATCH the credential record on mergen. Without this, manual-login
  // sessions in single-cred mode end up with an empty credential record on
  // the mergen side — captures get tagged with the credentialID but
