 ...
 *
 * Hackbrowser is intentionally decoupled from mergen's Provider system
 * (Karar 2 — Dependency Inversion, INTEGRATION.md §5). Resolution order:
 *   1. `override` parameter — used by mergen launcher which resolves
 *      via Provider and passes the result through `runCrawl({ model })`.
 *   2. ANTHROPIC_API_KEY env var
 *   3. OPENAI_API_KEY env var
 *
 * Standalone (`bun src/index.ts`) only sees env vars — `mergen auth
...
export async function resolveModel(override?: LanguageModel): Promise<LanguageModel> {
  if (override) {
    ...
    log.info("model resolved via opts.model (mergen injection)")
    return cachedModel
  }
