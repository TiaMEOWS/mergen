...
// (`bun src/index.ts`) and mergen-internal callers (Tool / Slash / CLI
...
// mergen launcher) can decide their own surface.
...
/**
 * Library API options. Flat shape (vs. AgentConfig's nested mergen/auth
 * blocks) — easier for mergen launcher and external callers.
 */
export interface CrawlOptions {
  // Target (required)
  url: string

  // Mergen integration — populated by launcher when invoked in-process
  sessionID?: string
  credentialID?: string
  ...
  // Mergen launcher uses this to forward into its own Log namespace.
  logSink?: LogSink
  ...
  // callback synchronously. Mergen launcher routes these into
  ...
  // Mergen ingest target (HTTP loopback — Karar 6, INTEGRATION.md §10.5)
  mergenUrl?: string
  mergenUsername?: string
  mergenPassword?: string
...
function toAgentConfig(opts: CrawlOptions): AgentConfig {
  return {
    targetUrl: opts.url,
    mergen: {
      serverUrl: opts.mergenUrl ?? "http://127.0.0.1:4096",
      sessionID: opts.sessionID,
      credentialId: opts.credentialID,
      username: opts.mergenUsername,
      password: opts.mergenPassword,
    },
    auth: {
...
/**
 * Run a crawl. Single entry point for both standalone CLI and mergen-
 * internal callers.
 *
 * Validation/preflight throw immediately. Runtime errors during the crawl
 * land in CrawlResult.errors[] — they don't propagate, so mergen's
 ...
 * (`bun src/index.ts`); mergen-internal callers construct opts directly.
 *
...
export function parseArgsToOptions(argv: string[]): CrawlOptions {
  ...
  return {
    ...
    mergenUrl: getArg("--mergen"),
    mergenUsername: getArg("--mergen-username"),
    mergenPassword: getArg("--mergen-password") ?? process.env.MERGEN_SERVER_PASSWORD,
