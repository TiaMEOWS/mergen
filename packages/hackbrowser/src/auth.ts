...
export async function waitForManualLogin(
  ...
  const callbackName = label ? `__mergenReady_${label}` : "__mergenReady"
  await page.exposeFunction(callbackName, () => {
    resolveReady()
  })

  const posCallbackName = label ? `__mergenPos_${label}` : "__mergenPos"
  ...
  const buttonId = label ? `__mergen-ready-btn-${label}` : "__mergen-ready-btn"
  const styleId = "__mergen-ready-btn-style"
