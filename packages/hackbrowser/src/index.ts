...
// parsing + a help message + an exit-code handoff. Mergen-internal
...
Options:
  ...
  --mergen <url>        Mergen server URL (default: http://127.0.0.1:4096)
  --mergen-username <u> Mergen server username (default: "mergen")
  --mergen-password <p> Mergen server password (fallback: MERGEN_SERVER_PASSWORD env var)
  --session-id <id>          Attach to existing Mergen session
  ...
  --dry-run                  Crawl without sending to Mergen; print captures to console
  --no-panel                 Disable the live telemetry panel injected into the browser
  --debug                    Enable verbose debug logging

Note: hackbrowser inside mergen (Tool/Slash) uses the same engine via
