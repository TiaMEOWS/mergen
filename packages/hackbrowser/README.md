# Mergen Browser Agent

AI-driven browser agent that automatically navigates web applications, captures HTTP requests with UI context, and sends them to Mergen for security analysis.
...
Playwright browser
    ...
    └── → POST /session/ingest to Mergen (with ui_context field)
```

The key addition over the Firefox extension: every captured request is enriched with `ui_context` — a snapshot of the page's form fields at the time the request was made. This tells Mergen:
...
# Attach to existing Mergen session
...
├── ingest.ts     # Send to Mergen /session/ingest
...
Mergen's proxy-analyzer and mass-assignment agents will use this to identify:
