# Mergen Browser Agent — Panel UI Brief
...
- `data-mergen-ui="panel"` attribute scanner tarafından filter edilir (§6)
...
export const PANEL_INIT_SCRIPT = `
  (function() {
    ...
    host.id = '__cs-host';
    host.setAttribute('data-mergen-ui', 'panel');
...
**Zorunlu filter:** Scanner'daki `INTERACTIVE_SELECTORS` aramasında `[data-mergen-ui]` alt ağacı ignore edilmeli.

```ts
// scanner.ts — içeride page.evaluate'da
for (const el of document.querySelectorAll(INTERACTIVE_SELECTORS)) {
  if (el.closest("[data-mergen-ui]")) continue // ← EKLENECEK
  // ... mevcut mantık
}
```
...
`collectInteractiveElements` evaluate içinde her element için `el.closest("[data-mergen-ui]")` kontrolü → skip.

### 8.4 `src/capture.ts` dokunuşu (~1 satır, defansif)

`snapshotPageUI` içinde input selector'a `:not([data-mergen-ui] *)` eklenebilir. Panel content area Shadow DOM olduğu için normalde ulaşılmaz, ama zararsız savunma.
