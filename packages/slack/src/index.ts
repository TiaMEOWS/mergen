...
import { createMergen, type ToolPart } from "@mergen-io/sdk"
...
console.log("🚀 Starting mergen server...")
const mergen = await createMergen({
  port: 0,
})
console.log("✅ Mergen server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await mergen.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new mergen session...")
    const { client, server } = mergen
    ...
    console.log("✅ Created mergen session:", createResult.data.id)
  ...
  console.log("📝 Sending to mergen:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Mergen response:", JSON.stringify(result, null, 2))
