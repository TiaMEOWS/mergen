<p align="center">
  <img src="https://raw.githubusercontent.com/TiaMEOWS/mergen/dev/assets/social-preview-dark.svg" alt="Mergen" width="800">
</p>
...
<p align="center">
  ...
  <a href="https://docs.mergen.dev">Docs</a> &bull;
  <a href="https://mergen.dev">Website</a>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@mergen-io/mergen"><img alt="npm" src="https://img.shields.io/npm/v/@mergen-io/mergen?style=flat-square&color=00ff41" /></a>
  <a href="https://www.npmjs.com/package/@mergen-io/mergen"><img alt="Downloads" src="https://img.shields.io/npm/dm/@mergen-io/mergen?style=flat-square&color=00ff41" /></a>
  <a href="https://github.com/TiaMEOWS/mergen/actions/workflows/publish.yml"><img alt="Build" src="https://img.shields.io/github/actions/workflow/status/TiaMEOWS/mergen/publish.yml?style=flat-square&branch=dev" /></a>
  <a href="https://discord.gg/snunAaHf6U"><img alt="Discord" src="https://img.shields.io/discord/1391832426048651334?style=flat-square&label=discord&color=00ff41" /></a>
  <a href="https://github.com/TiaMEOWS/mergen/blob/dev/LICENSE"><img alt="License" src="https://img.shields.io/badge/license-AGPL--3.0-00ff41?style=flat-square" /></a>
</p>
...
```bash
npm i -g @mergen-io/mergen@latest && mergen
```

That's it. Mergen launches a TUI in your terminal, asks for your LLM provider and API key on first run, and you're ready to go. Tell it what to test — it loads relevant skills from 7,300+ MITRE ATT&CK, CIS, OWASP, and NIST test procedures, then handles reconnaissance, vulnerability discovery, exploitation, and reporting autonomously.

> **Already have a Claude Code or OpenAI subscription?** Mergen's intelligence layer sits on top of your existing AI subscription. No separate API costs — your current plan powers an entire pentest toolkit.

Explore the full documentation at **[docs.mergen.dev](https://docs.mergen.dev)** or visit **[mergen.dev](https://mergen.dev)** for demos and guides.

---

### Intelligence Layer

Mergen isn't just a wrapper around an LLM. It's an intelligence layer that transforms any AI model into an offensive security specialist.

**How it works:** When you connect your LLM provider, Mergen injects domain-specific context — OWASP testing methodology, vulnerability patterns, attack chain reasoning, and tool orchestration logic — into every interaction. The model doesn't need to know security; Mergen teaches it.
...
> **Air-gapped environments?** Run Mergen entirely offline with Ollama or LM Studio. No data leaves your machine — ever.

---

### Skills System — 7,300+ Actionable Security Tests

Mergen doesn't just give agents generic security knowledge — it loads domain-specific skills on-demand with zero context pollution.
...
Read more: [MITRE ATT&CK Integration](https://github.com/TiaMEOWS/mergen/blob/main/.mergen/skill/mitre_attack/), [CIS Benchmarks](https://github.com/TiaMEOWS/mergen/tree/main/.mergen/skill/CIS_benchmarks)
...
Most AI security tools are thin wrappers that send your prompt to an API. Mergen's intelligence layer normalizes outputs across 15+ providers, guards context between test phases, auto-detects your provider configuration, and orchestrates multi-step attack chains. The result: consistent, methodology-driven pentesting regardless of which model you use.
...
Anthropic, OpenAI, Google, Amazon Bedrock, Azure, Groq, Mistral, DeepSeek, OpenRouter, Together AI — or run fully offline with Ollama and LM Studio. You choose the model. You own the results. As AI models get better and cheaper, Mergen gets better with them. Switch providers in seconds without reconfiguring anything.
...
Your security tools don't have to run on your laptop. Deploy Bolt on one or many remote servers, pair with Ed25519 keys, and control everything from your local terminal. One Mergen instance can orchestrate dozens of Bolt servers — each with its own toolkit, network position, and attack surface access.
...
| ---------------------- | ------- | ------------------------------------------------------------------- |
| **mergen**        | General | Full-access primary agent — reconnaissance, exploitation, reporting |
...
Run `mergen web` and control your agents, MCP servers, Bolt connections, and vulnerability findings from any browser. Access from anywhere with Cloudflare Tunnel — zero open ports, end-to-end encryption, password-protected API. Your data stays on your machine.

```bash
export MERGEN_SERVER_PASSWORD=your-secure-password
mergen web
```

Use **[app.mergen.dev](https://app.mergen.dev)** (static page, no backend) or self-host from `packages/app/dist/`.

See the [full README](https://github.com/TiaMEOWS/mergen#web-ui--remote-access) for the complete security model.

---

### Bolt — Remote Tool Execution

Bolt is Mergen's remote tool server. Deploy it on any VPS, cloud instance, or Docker container — then control it from your local terminal over MCP protocol with Ed25519 authentication.

**One Mergen, many Bolt servers:**
...
- **Manage from TUI** — Add, remove, and monitor Bolt servers without leaving Mergen
...
Mergen connects to specialized MCP servers that extend its capabilities — **176+ security tools** across 5 domains:
...
All open source. All installable with `npx`. Plug them into Mergen or use them standalone with any MCP-compatible client.

---

### Built-in Tools

Mergen agents have direct access to 30+ tools without any external dependencies:
...
```bash
# npm (recommended)
npm i -g @mergen-io/mergen@latest

# bun / pnpm / yarn
bun add -g @mergen-io/mergen@latest

# macOS (Homebrew)
brew install TiaMEOWS/tap/mergen

# Windows (Scoop)
scoop install mergen

# Linux / macOS (curl)
curl -fsSL https://mergen.dev/install.sh | bash
```
...
- **Bug Bounty Hunters** — Faster reconnaissance, wider coverage, consistent methodology across programs. Mergen doesn't get tired at 3am.
...
- **Security Researchers** — Extend Mergen with custom agents and MCP servers. The plugin system and MCP protocol make it a platform, not just a tool.

---

### Contributing

Mergen is built by the security community, for the security community. We welcome contributions across:
...
Read the [Contributing Guide](https://github.com/TiaMEOWS/mergen/blob/dev/CONTRIBUTING.md) before submitting a PR. All contributions must follow the project's [ethical use policy](https://github.com/TiaMEOWS/mergen/blob/dev/CODE_OF_CONDUCT.md) — Mergen is for authorized security testing only.

---

### License

[AGPL-3.0-only](https://github.com/TiaMEOWS/mergen/blob/dev/LICENSE) — Free for personal and open-source use. Commercial licensing available via [contact@mergen.dev](mailto:contact@mergen.dev).

---

### MCP Security Suite

Mergen is the core platform. These MCP servers extend its capabilities:

| Project                                                                | Domain                                  | Tools                                       |
| ---------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------- |
| **Mergen**                                                        | **Autonomous offensive security agent** | **7,300+ skills (MITRE, CIS, OWASP, NIST)** |
...
<p align="center">
  <a href="https://mergen.dev"><b>mergen.dev</b></a> · <a href="https://docs.mergen.dev"><b>Docs</b></a> · <a href="https://discord.gg/snunAaHf6U"><b>Discord</b></a> · <a href="https://x.com/mergenio"><b>X.com</b></a> · <a href="https://github.com/TiaMEOWS/mergen"><b>GitHub</b></a>
</p>
<p align="center">
