...
test("discovers skills from .mergen/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".mergen", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".mergen", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
...
`,
  ...
  process.env.MERGEN_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".mergen", "skill", "dir-skill")
...
test("discovers multiple skills from .mergen/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".mergen", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".mergen", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".mergen", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.MERGEN_TEST_HOME
  process.env.MERGEN_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.MERGEN_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const mergenSkillDir = path.join(dir, ".mergen", "skill", "agent-skill")
      const mergenSkillsDir = path.join(dir, ".mergen", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(mergenSkillDir, "SKILL.md"),
        `---
name: mergen-skill
description: A skill in the .mergen/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(mergenSkillsDir, "SKILL.md"),
        `---
name: mergen-skill
description: A skill in the .mergen/skills directory.
---
