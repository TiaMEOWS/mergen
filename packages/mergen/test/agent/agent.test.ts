...
test("mergen agent has correct default properties", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const cs = await Agent.get("mergen")
...
test("custom agent config overrides native agent properties", async () => {
    ...
    config: {
      agent: {
        mergen: {
          ...
          description: "Custom mergen agent",
          temperature: 0.7,
          color: "#FF0000",
        },
      },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(cs?.description).toBe("Custom mergen agent")
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        mergen: { prompt: "Custom system prompt" },
      },
    },
  })
  await Instance.provide({
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".mergen", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const cs = await Agent.get("mergen")
        const skillDir = path.join(tmp.path, ".mergen", "skill", "perm-skill")
...
test("defaultAgent returns mergen when no default_agent config", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const agent = await Agent.defaultAgent()
      expect(agent).toBe("mergen")
    },
  })
})
...
test("defaultAgent returns next primary when mergen is disabled", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        mergen: { disable: true },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await Agent.defaultAgent()
      // mergen is disabled, so it should return the next primary agent
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // mergen is the only primary agent, disabled = no primary visible agents remain
