...
const managedConfigDir = process.env.MERGEN_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "mergen.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "mergen.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "mergen.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "mergen.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .mergen directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const mergenDir = path.join(dir, ".mergen")
      await fs.mkdir(mergenDir, { recursive: true })
      const agentDir = path.join(mergenDir, "agent")
...
test("loads agents from .mergen/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const mergenDir = path.join(dir, ".mergen")
      await fs.mkdir(mergenDir, { recursive: true })

      const agentsDir = path.join(mergenDir, "agents")
...
test("loads commands from .mergen/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const mergenDir = path.join(dir, ".mergen")
      await fs.mkdir(mergenDir, { recursive: true })

      const commandDir = path.join(mergenDir, "command")
...
test("loads commands from .mergen/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const mergenDir = path.join(dir, ".mergen")
      await fs.mkdir(mergenDir, { recursive: true })

      const commandsDir = path.join(mergenDir, "commands")
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = JSON.parse(await Bun.file(path.join(tmp.path, "mergen.json")).text())
...
test("does not try to install dependencies in read-only MERGEN_CONFIG_DIR", async () => {
  ...
  const prev = process.env.MERGEN_CONFIG_DIR
  process.env.MERGEN_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MERGEN_CONFIG_DIR
    else process.env.MERGEN_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MERGEN_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "mergen.json"),
        JSON.stringify({ $schema: "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "mergen.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .mergen config
      const projectDir = path.join(dir, "project")
      const mergenDir = path.join(projectDir, ".mergen")
      await fs.mkdir(mergenDir, { recursive: true })
      ...
      // Local .mergen config with different plugins
      await Bun.write(
        path.join(mergenDir, "mergen.json"),
        JSON.stringify({
          $schema: "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mergen config with some overlapping plugins
      await Bun.write(
        path.join(mergenDir, "mergen.json"),
        JSON.stringify({
          $schema: "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json",
...
// Note: preload.ts sets MERGEN_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .mergen config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mergen directory config enables it
      const mergenDir = path.join(dir, ".mergen")
      await fs.mkdir(mergenDir, { recursive: true })
      await Bun.write(
        path.join(mergenDir, "mergen.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/mergen")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/mergen")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-mergen@2.4.3")).toBe("oh-my-mergen")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@mergen/plugin@2.0.0")).toBe("@mergen/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-mergen@2.4.3", "file:///project/.mergen/plugin/oh-my-mergen.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.mergen/plugin/oh-my-mergen.js")
  })
  ...
  test("local plugin directory overrides global mergen.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const mergenDir = path.join(projectDir, ".mergen")
        const pluginDir = path.join(mergenDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("MERGEN_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MERGEN_DISABLE_PROJECT_CONFIG"]
    process.env["MERGEN_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MERGEN_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MERGEN_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .mergen/ directories when flag is set", async () => {
    const originalEnv = process.env["MERGEN_DISABLE_PROJECT_CONFIG"]
    process.env["MERGEN_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .mergen directory with a command
          const mergenDir = path.join(dir, ".mergen", "command")
          await fs.mkdir(mergenDir, { recursive: true })
          await Bun.write(path.join(mergenDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .mergen should NOT be in directories list
          const hasProjectMergen = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectMergen).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MERGEN_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MERGEN_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MERGEN_CONFIG_DIR"]
      process.env["MERGEN_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MERGEN_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MERGEN_CONFIG_DIR"]
      } else {
        process.env["MERGEN_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MERGEN_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MERGEN_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MERGEN_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MERGEN_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MERGEN_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from MERGEN_CONFIG_DIR, not project
