...
// Usage:
//   First time:  bun run packages/mergen/script/sign-skills.ts --generate
//   Re-sign:     bun run packages/mergen/script/sign-skills.ts
//
// Reads private key from MERGEN_SKILL_PRIVATE_KEY env var or .skill-signing-key file
// The .skill-signing-key file is gitignored — NEVER commit it
import path from "path"

const root = path.resolve(import.meta.dir, "../../..")
const skillDir = path.join(root, ".mergen", "skill")
const keyFile = path.join(root, ".skill-signing-key")
const signingModule = path.join(root, "packages", "mergen", "src", "skill", "signing.ts")
...
async function main() {
  ...
  if (generate) {
    ...
    privateKeyB64 = process.env.MERGEN_SKILL_PRIVATE_KEY ?? ""
    if (!privateKeyB64) {
      const exists = await Bun.file(keyFile).exists()
      if (!exists) {
        console.error("No private key found. Run with --generate first, or set MERGEN_SKILL_PRIVATE_KEY env var.")
        process.exit(1)
      }
