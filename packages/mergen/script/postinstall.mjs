...
function findBinary() {
  ...
  const packageName = `@mergen-io/mergen-${platform}-${arch}`
  const binaryName = platform === "windows" ? "mergen.exe" : "mergen"
...
function symlinkBinary(sourcePath, binaryName) {
  ...
  console.log(`mergen binary symlinked: ${targetPath} -> ${sourcePath}`)
...
function installWebUI() {
  ...
  const dataDir = path.join(xdgDataDir(), "mergen")
...
// packages/hackbrowser/package.json (and packages/mergen/package.json). The
  ...
  if (!fs.existsSync(playwrightDir)) {
    ...
    try {
      ...
      console.warn(
        ...
        err.message,
        `\nTo install manually: npm install --prefix ~/.local/share/mergen --save-exact playwright@${PLAYWRIGHT_VERSION}`,
      )
    }
  } else {
 ...
 * Corporate installs often skip PATH setup, leaving `mergen` unreachable
  ...
  try {
    ...
    console.warn(
      `Warning: Could not add npm global bin to PATH automatically.\n` +
        `If 'mergen' command is not found, add npm's global bin directory to your PATH manually:\n` +
...
async function main() {
  try {
    ...
    console.error("Failed to setup mergen binary:", error.message)
    process.exit(1)
  }
}
