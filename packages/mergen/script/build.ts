...
const modelsUrl = process.env.MERGEN_MODELS_URL || "https://models.dev"
...
const buildOSRaw = process.env.MERGEN_BUILD_OS?.trim()
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/mergen`,
      execArgv: [`--user-agent=mergen/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    entrypoints: ["./src/index.ts", parserWorker, workerPath],
    define: {
      MERGEN_VERSION: `'${Script.version}'`,
      MERGEN_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
      MERGEN_WORKER_PATH: workerPath,
      MERGEN_CHANNEL: `'${Script.channel}'`,
      MERGEN_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
    },
  })
...
// ~/.local/share/mergen/node_modules/ and are installed by postinstall.
