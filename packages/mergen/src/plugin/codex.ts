import type { Hooks, PluginInput } from "@mergen-io/plugin"
...
<html>
  <head>
    <title>Mergen - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to Mergen.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>Mergen - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput): Promise<Hooks> {
  return {
          ...
          authorize: async () => {
            const deviceResponse = await fetch(`${ISSUER}/api/accounts/deviceauth/usercode`, {
              ...
              headers: {
                ...
                "User-Agent": `mergen/${Installation.VERSION}`,
              },
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "mergen"
      output.headers["User-Agent"] =
        `mergen/${Installation.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`
