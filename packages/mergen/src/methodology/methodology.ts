...
export namespace Methodology {
  ...
  export function getPhaseDirectives(phaseId: Phase.Id): string {
    const directives: Record<Phase.Id, string> = {
      ...
      reporting:
        "Call `generate_report` to compile all session data into a structured Markdown report. Review the output, then write three AI sections: Executive Summary (overall risk posture), Risk Assessment (likelihood × impact matrix), and Remediation Priorities (actionable steps). Replace the `<!-- AI: ... -->` placeholders and save to `.mergen/reports/report-<timestamp>.md` via the write tool. Ensure all blocking violations are resolved before finalizing.",
    }
