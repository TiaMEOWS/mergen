...
export namespace SessionPrompt {
      ...
      if (disabledServers.length > 0) {
        system.push(
          [
            ...
            "If the user asks about capabilities these servers provide, inform them that the relevant MCP server is available but disabled, and they can enable it via the MCP panel or `mergen mcp enable <name>`.",
          ].join("\n"),
        )
      }
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (!Flag.MERGEN_EXPERIMENTAL_PLAN_MODE) {
