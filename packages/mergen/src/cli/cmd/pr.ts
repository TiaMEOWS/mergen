...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run mergen",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for mergen session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found mergen session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`mergen import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting mergen...")
        UI.println()

        // Launch mergen TUI with session ID if available
        const { spawn } = await import("child_process")
        const mergenArgs = sessionId ? ["-s", sessionId] : []
        const mergenProcess = spawn("mergen", mergenArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          mergenProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`mergen exited with code ${code}`))
          })
          mergenProcess.on("error", reject)
        })
      },
    })
  },
