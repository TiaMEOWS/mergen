  ...
  async handler() {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          prompts.log.warn("No MCP servers configured")
          prompts.outro("Add servers with: mergen mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in mergen.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .mergen/ subdirectory too)
  const candidates = [path.join(baseDir, "mergen.json"), path.join(baseDir, "mergen.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".mergen", "mergen.json"),
      path.join(baseDir, ".mergen", "mergen.jsonc"),
    )
  }
  ...
  // Default to mergen.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., mergen x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "mergen-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
