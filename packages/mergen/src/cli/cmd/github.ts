...
const AGENT_USERNAME = "mergen-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/mergen.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://mergen.dev/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/mergen-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.mergen.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  mergen:
    if: |
      ...
      startsWith(github.event.comment.body, '/mergen')
    ...
    steps:
      ...
      - name: Run mergen
        uses: TiaMEOWS/mergen/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.mergen.dev" : "https://mergen.dev"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.mergen.dev"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/mergen,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        ...
        const result = await SessionPrompt.prompt({
          ...
          // agent is omitted - server will use default_agent from config or fall back to "mergen"
          parts: [
            {
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("mergen-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `mergen/${type}-${hex}-${timestamp}`
        }
        return `mergen/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/mergen-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[mergen session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the mergen infrastructure after your response",
