...
import { EOL } from "os"
import { createMergenClient, type Message, type MergenClient, type ToolPart } from "@mergen-io/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run mergen with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running mergen server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: MergenClient) {
    ...
    async function share(sdk: MergenClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.MERGEN_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: MergenClient) {
    ...
    if (args.attach) {
      const sdk = createMergenClient({ baseUrl: args.attach, directory })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      if (port !== undefined) {
        ...
        const sdk = createMergenClient({ baseUrl, directory: process.cwd() })
        return await execute(sdk)
      }
      ...
      const sdk = createMergenClient({ baseUrl: "http://mergen.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
