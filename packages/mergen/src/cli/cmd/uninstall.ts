...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall mergen and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Mergen")

    // Check for other running mergen instances
    const running = await getRunningInstances()
    if (running > 0) {
      prompts.log.warn(`${running} other mergen process${running > 1 ? "es" : ""} running`)
      if (!args.force) {
        ...
        if (!proceed || prompts.isCancel(proceed)) {
          prompts.outro("Stop other mergen processes first, then retry")
          return
        }
      }
    }
...
async function getRunningInstances(): Promise<number> {
  ...
  try {
    if (process.platform === "win32") {
      const result = await $`tasklist /FI "IMAGENAME eq mergen.exe" /FO CSV /NH`.quiet().nothrow()
      const lines = result.stdout
        .toString("utf8")
        .split("\n")
        .filter((l) => l.includes("mergen"))
      return Math.max(0, lines.length - 1)
    }
    const result = await $`pgrep -f mergen`.quiet().nothrow()
...
async function collectRemovalTargets(args: UninstallArgs, method: Installation.Method): Promise<RemovalTargets> {
  const directories: RemovalTargets["directories"] = [
    ...
    },
    {
      ...
      label: "Config",
      description: "mergen.json, provider settings",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @mergen-io/mergen",
      pnpm: "pnpm uninstall -g @mergen-io/mergen",
      bun: "bun remove -g @mergen-io/mergen",
      yarn: "yarn global remove @mergen-io/mergen",
      brew: "brew uninstall mergen",
      choco: "choco uninstall mergen",
      scoop: "scoop uninstall mergen",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (dataTarget && !dataTarget.keep) {
    const dbPath = path.join(dataTarget.path, "mergen.db")
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall mergen -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.info(
    `Per-project ${UI.Style.TEXT_NORMAL_BOLD}.mergen/${UI.Style.TEXT_NORMAL} directories (skill cache, project config) are not removed.`,
  )
  prompts.log.info(`  To find them: ${UI.Style.TEXT_DIM}find ~ -name ".mergen" -type d -maxdepth 5`)

  UI.empty()
  prompts.log.success("Thank you for using Mergen!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# mergen") || content.includes(".mergen/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".mergen/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".mergen/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".mergen"))
    ) {
      continue
    }
