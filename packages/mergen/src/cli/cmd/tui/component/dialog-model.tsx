...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "mergen" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
        ...
        return [
          {
            ...
            category,
            disabled: provider.id === "mergen" && model.id.includes("-nano"),
            footer: model.cost?.input === 0 && provider.id === "mergen" ? "Free" : undefined,
            onSelect: () => {
              dialog.clear()
      ...
      flatMap((provider) =>
        pipe(
          ...
          map(([model, info]) => ({
            ...
            disabled: provider.id === "mergen" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "mergen" ? "Free" : undefined,
            onSelect() {
              dialog.clear()
