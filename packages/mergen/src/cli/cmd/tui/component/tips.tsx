...
const TIPS = [
  ...
  "Mergen runs {highlight}120+ OWASP test cases{/highlight} autonomously across web targets",
  "Use specialized agents for {highlight}recon{/highlight}, {highlight}exploitation{/highlight}, and {highlight}reporting{/highlight}",
  "Mergen generates {highlight}HackerOne-ready{/highlight} vulnerability reports from verified findings",
  ...
  "Mergen supports {highlight}15+ vulnerability skills{/highlight}: IDOR, SSRF, XSS, injection, misconfig, and more",
  ...
  "Mergen supports {highlight}HAR replay{/highlight} — replay captured traffic with modifications",
  ...
  // Local LLM & provider
  "Run {highlight}mergen provider add{/highlight} to connect local LLMs (vLLM, Ollama, llama.cpp)",
  ...
  // Config & agents
  "Add {highlight}.md{/highlight} files to {highlight}.mergen/agent/{/highlight} for specialized security personas",
  ...
  "Use {highlight}mergen run{/highlight} for non-interactive scripting and CI/CD integration",
  "Create {highlight}.ts{/highlight} files in {highlight}.mergen/tools/{/highlight} to define custom LLM tools",
