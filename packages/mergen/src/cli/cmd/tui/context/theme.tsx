...
import mergen from "./theme/mergen.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (sync.data.config.theme ?? kv.get("theme", "mergen")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.mergen, store.mode)
    })
