...
export async function ec2Snapshot(args: string[], timeout: number): Promise<HookResult> {
  ...
  const r = await aws(
    [
      ...
      volumeId,
      "--description",
      "Mergen forensic snapshot",
      "--tag-specifications",
      "ResourceType=snapshot,Tags=[{Key=CreatedBy,Value=Mergen}]",
    ],
    profile,
    region,
    timeout,
...
export async function s3Exfil(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (put.exitCode === 0) {
    ...
    findings.push({
      ...
      remediation: "Remove MergenExfil statement from bucket policy",
    })
  } else {
...
export async function cleanupAws(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output = [`[*] Mergen AWS cleanup — ${mode}\n`]
  ...
  const snaps = await aws(
    [
      ...
      "self",
      "--filters",
      "Name=tag:CreatedBy,Values=Mergen",
      "--query",
