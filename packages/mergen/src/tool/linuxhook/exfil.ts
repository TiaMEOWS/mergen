...
export async function cleanupLinux(args: string[], timeout: number): Promise<HookResult> {
  const findings: Finding[] = []
  const output: string[] = ["=== Mergen Linux Cleanup ==="]
...
echo ""
echo "--- Cron Entries ---"
if crontab -l 2>/dev/null | grep -q "cs_\\|mergen"; then
  echo "[+] Found Mergen cron entries"
  crontab -l 2>/dev/null | grep "cs_\\|mergen"
  if [ "$DRY" = "0" ]; then
    crontab -l 2>/dev/null | grep -v "cs_\\|mergen" | crontab - 2>/dev/null
...
for dir in /root /home/*; do
  if [ -f "$dir/.ssh/authorized_keys" ]; then
    if grep -q "mergen\\|cs_implant" "$dir/.ssh/authorized_keys" 2>/dev/null; then
      echo "[+] Found Mergen key in $dir/.ssh/authorized_keys"
      if [ "$DRY" = "0" ]; then
        grep -v "mergen\\|cs_implant" "$dir/.ssh/authorized_keys" > "$dir/.ssh/authorized_keys.tmp"
...
for dir in /root /home/*; do
  for rc in .bashrc .bash_profile .profile .zshrc; do
    if [ -f "$dir/$rc" ] && grep -q "cs_\\|mergen" "$dir/$rc" 2>/dev/null; then
      echo "[+] Found injection in $dir/$rc"
      if [ "$DRY" = "0" ]; then
        grep -v "cs_\\|mergen" "$dir/$rc" > "$dir/$rc.tmp"
...
echo ""
echo "--- Udev Rules ---"
for f in /etc/udev/rules.d/cs_* /etc/udev/rules.d/*mergen*; do
  if [ -e "$f" ] 2>/dev/null; then
    echo "[+] Found: $f"
    [ "$DRY" = "0" ] && rm -f "$f" && echo "    Removed" && CLEANED=$((CLEANED+1))
  fi
...
echo ""
echo "--- APT Hooks ---"
for f in /etc/apt/apt.conf.d/cs_* /etc/apt/apt.conf.d/*mergen*; do
  if [ -e "$f" ] 2>/dev/null; then
    echo "[+] Found: $f"
    [ "$DRY" = "0" ] && rm -f "$f" && echo "    Removed" && CLEANED=$((CLEANED+1))
  fi
...
if [ "$DRY" = "0" ]; then
  ...
  for dir in /root /home/*; do
    for h in .bash_history .zsh_history; do
      if [ -f "$dir/$h" ]; then
        grep -v "linuxhook\\|mergen\\|cs_" "$dir/$h" > "$dir/$h.tmp" 2>/dev/null
...
if [ "$DRY" = "0" ]; then
  for log in /var/log/auth.log /var/log/secure /var/log/syslog; do
    if [ -f "$log" ] && [ -w "$log" ]; then
      grep -v "linuxhook\\|mergen\\|cs_" "$log" > "$log.tmp" 2>/dev/null
...
`
  ...
  findings.push({
    ...
    resource: "system",
    title: dryRun ? "Cleanup dry run completed" : "Mergen artifacts cleaned",
    details: dryRun
...
export async function artifactEnum(args: string[], timeout: number): Promise<HookResult> {
  const findings: Finding[] = []
  const output: string[] = ["=== Mergen Artifact Enumeration ==="]

  const script = `
echo "[*] Scanning for Mergen artifacts on this system..."
echo ""
TOTAL=0
...
echo ""
echo "--- Cron ---"
crontab -l 2>/dev/null | grep -n "cs_\|mergen" && TOTAL=$((TOTAL+1))
ls /etc/cron.d/cs_* /etc/cron.d/*mergen* 2>/dev/null && TOTAL=$((TOTAL+1))
...
echo "--- SSH Keys ---"
for dir in /root /home/*; do
  grep -l "mergen\|cs_implant" "$dir/.ssh/authorized_keys" 2>/dev/null && TOTAL=$((TOTAL+1))
done
...
for dir in /root /home/*; do
  for rc in .bashrc .bash_profile .profile .zshrc; do
    grep -l "cs_\|mergen" "$dir/$rc" 2>/dev/null && TOTAL=$((TOTAL+1))
  done
done

echo ""
echo "--- Udev / APT / PAM ---"
ls /etc/udev/rules.d/cs_* /etc/udev/rules.d/*mergen* 2>/dev/null && TOTAL=$((TOTAL+1))
ls /etc/apt/apt.conf.d/cs_* /etc/apt/apt.conf.d/*mergen* 2>/dev/null && TOTAL=$((TOTAL+1))
ls /lib/security/cs_* /lib/x86_64-linux-gnu/security/cs_* 2>/dev/null && TOTAL=$((TOTAL+1))

echo ""
echo "--- ld.so.preload ---"
grep -l "cs_\|mergen" /etc/ld.so.preload 2>/dev/null && TOTAL=$((TOTAL+1))

echo ""
echo "--- Log Traces ---"
grep -cl "linuxhook\|mergen\|cs_" /var/log/auth.log /var/log/secure /var/log/syslog 2>/dev/null && TOTAL=$((TOTAL+1))
...
`
  ...
  findings.push({
    ...
    resource: "system",
    title: total > 0 ? `${total} Mergen artifact(s) detected` : "No artifacts found",
    details:
      total > 0
        ? `${total} artifact(s) found on system — run cleanup_linux to remove`
        : "System appears clean of Mergen artifacts",
