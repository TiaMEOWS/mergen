...
const PROGRAMS = {
  ...
  cleanup_container: {
    description:
      "Remove all Mergen-created containers, images, volumes, and networks (by label mergen=true). ALWAYS run before leaving",
    args: "[--dry-run]",
  },
...
async function imageBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  const create = await run(
    "docker",
    ["run", "-d", "--name", containerId, "--label", "mergen=true", base, "sh", "-c", "sleep 10"],
    timeout,
  )
  ...
  const commit = await run(
    "docker",
    ["commit", "--change", `ENTRYPOINT ["/entrypoint.sh"]`, "--change", `LABEL mergen=true`, containerId, name],
    timeout,
  )
...
async function cleanupContainer(_args: string[], timeout: number): Promise<HookResult> {
  ...
  const findings: Finding[] = []
  const output: string[] = ["[*] Cleaning up Mergen container resources...\n"]

  const containers = await run("docker", ["ps", "-a", "--filter", "label=mergen=true", "-q"], timeout)
  if (containers.exitCode === 0) {
    const ids = containers.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] Mergen containers: ${ids.length}`)
  ...
  const images = await run("docker", ["images", "--filter", "label=mergen=true", "-q"], timeout)
  if (images.exitCode === 0) {
    const ids = images.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] Mergen images: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
  ...
  const volumes = await run("docker", ["volume", "ls", "--filter", "label=mergen=true", "-q"], timeout)
  if (volumes.exitCode === 0) {
    const ids = volumes.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] Mergen volumes: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
  ...
  const networks = await run("docker", ["network", "ls", "--filter", "label=mergen=true", "-q"], timeout)
  if (networks.exitCode === 0) {
    const ids = networks.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] Mergen networks: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
