...
import { Sarif, Pdf } from "../session/report-export"
...
export const GenerateReportTool = Tool.define("generate_report", {
  ...
  parameters: z.object({
    ...
    format: z
      .enum(["markdown", "sarif", "pdf"])
      .optional()
      .describe(
        "markdown (default) returns the report body only. sarif/pdf ALSO write a machine/archival artifact: report.sarif (GitHub Code Scanning compatible) or report.pdf under out_dir / .mergen/findings/<session>/. Only confirmed, non-duplicate findings enter the SARIF run.",
      ),
    out_dir: z.string().optional().describe("Artifact directory for sarif/pdf (default: .mergen/findings/<session>)"),
  }),
  async execute(params, ctx) {
    ...
    parts.push(
      "---\n\n<!-- AI: remediation_priorities -->\n\n---\n\n*Generated by Mergen — github.com/TiaMEOWS/mergen*",
    )

    if (params.format === "sarif" || params.format === "pdf") {
      const base = params.out_dir ?? path.join(Instance.directory, ".mergen", "findings", rootSession)
      await fs.mkdir(base, { recursive: true })
      const file = path.join(base, params.format === "sarif" ? "report.sarif" : "report.pdf")
      if (params.format === "sarif") {
        await fs.writeFile(file, Sarif.build(vulns, { toolVersion: Installation.VERSION }), "utf8")
      } else {
        await fs.writeFile(file, Pdf.build(`Mergen security report -- ${session.title}`, parts.join("\n\n")))
      }
