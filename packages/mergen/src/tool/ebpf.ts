...
const AVAILABLE_PROGRAMS: Record<string, { description: string; args: string }> = {
  ...
  cleanup: {
    description:
      "Enumerate and detach all Mergen eBPF programs and maps from the system. Always run this before exiting a target",
    args: "[--json-output]",
  },
  io_uring_sniff: {
    description:
  ...
  bpf_integrity: {
    description:
      "Monitor bpf() syscall for program load/attach/detach operations and verify integrity of Mergen eBPF programs — detect tampering, unauthorized BPF program injection, and hook evasion",
