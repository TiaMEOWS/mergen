...
export async function artifactEnum(_args: string[], timeout: number): Promise<HookResult> {
  ...
  for (const dir of agentDirs) {
    const find = await run("find", [dir, "-name", "*mergen*", "-o", "-name", "*cs-*"], timeout)
  ...
  const tmp = await run(
    "find",
    ["/tmp", "-maxdepth", "1", "(", "-name", "cs-*", "-o", "-name", "mergen-*", ")"],
    timeout,
  )
  ...
  output.push("\n--- Running Processes ---")
  const procs = await run("pgrep", ["-fl", "mergen|cs-"], timeout)
  ...
  for (const hist of histFiles) {
    if (await Bun.file(hist).exists()) {
      const grep = await run("grep", ["-c", "mergen\\|machook", hist], timeout)
      if (grep.exitCode === 0) {
  ...
  if (total === 0) output.push("[+] No Mergen artifacts detected — target appears clean")
  ...
  findings.push({
    ...
    details: `${total} Mergen artifacts detected across LaunchAgents, temp files, processes, history, and databases`,
...
export async function cleanupMac(_args: string[], timeout: number): Promise<HookResult> {
  const findings: Finding[] = []
  const output: string[] = ["[*] Cleaning up Mergen artifacts from macOS target...\n"]
  const home = process.env.HOME || "/root"
  let cleaned = 0
  ...
  const csProcesses = await run("pgrep", ["-fl", "mergen|cs-hook|cs-monitor"], timeout)
  ...
  for (const hist of historyFiles) {
    ...
    if (await Bun.file(histPath).exists()) {
      ...
      const filtered = content
        .split("\n")
        .filter((l) => !l.includes("mergen") && !l.includes("machook") && !l.includes("cs-"))
        .join("\n")
      if (filtered.length !== content.length) {
        await Bun.write(histPath, filtered)
        output.push(`[+] Scrubbed Mergen entries from ${hist}`)
        cleaned++
      }
    }
  }
  ...
  if (cleaned === 0) output.push("[*] No Mergen artifacts found — target is clean")
  ...
  findings.push({
    ...
    details: `Removed ${cleaned} Mergen artifacts (LaunchAgents, processes, temp files, DTrace scripts, shell history)`,
    remediation: "Verify no traces remain with: find / -name '*mergen*' 2>/dev/null",
  })
