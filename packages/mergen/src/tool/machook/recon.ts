...
export async function installedApps(_args: string[], timeout: number): Promise<HookResult> {
  ...
  findings.push({
    ...
    details: `${appCount} applications found. Review application names and versions in the output — for any with a version, check CVE database via cve-mcp (cve search_by_product --product <name> --version <ver>). If cve-mcp is not enabled: mergen mcp enable cve`,
