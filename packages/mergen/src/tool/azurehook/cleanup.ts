...
export async function cleanupAzure(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output: string[] = [
    dryRun
      ...
      : "[*] Cleaning up Mergen artifacts from Azure...\n",
  ]
  let cleaned = 0

  output.push("[*] Checking for Mergen automation runbooks (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for Mergen role assignments...")
  ...
  if (assignments.exitCode === 0) {
    ...
    for (const r of roles) {
      ...
      if (!desc.includes("mergen") && !desc.includes("cs-")) continue
      if (dryRun) {
  ...
  output.push("\n[*] Checking for Mergen app registrations (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for Mergen snapshots (cs-snap-* prefix)...")
  const snapshots = await az(["snapshot", "list", "--query", "[?tags.mergen=='true']"], sub, timeout)
  ...
  output.push("\n[*] Checking for Mergen policy exemptions (cs-exempt-* prefix)...")
  ...
  output.push("\n[*] Checking for Mergen Event Grid subscriptions (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for Mergen Event Hub consumer groups (cs-tap-* prefix)...")
  ...
  output.push("\n[*] Checking for Mergen Lighthouse assignments...")
