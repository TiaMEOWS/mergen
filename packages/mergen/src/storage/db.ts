...
declare const MERGEN_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export namespace Database {
  ...
  export const Client = lazy(() => {
    log.info("opening database", { path: path.join(Global.Path.data, "mergen.db") })

    const sqlite = new BunDatabase(path.join(Global.Path.data, "mergen.db"), { create: true })
    ...
    const entries =
      typeof MERGEN_MIGRATIONS !== "undefined"
        ? MERGEN_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof MERGEN_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
      // Make CREATE statements idempotent so migrations survive re-install over an
      // existing database (e.g. `mergen uninstall` removes the binary but leaves
