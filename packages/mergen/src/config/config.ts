...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/mergen"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "mergen")
      default:
        return "/etc/mergen"
    }
  }

  const managedConfigDir = process.env.MERGEN_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  export const state = Instance.state(async () => {
    ...
    // 4) Project config (mergen.json{,c})
    // 5) .mergen directories (.mergen/agents/, .mergen/commands/, .mergen/plugins/, .mergen/mergen.json{,c})
    // 6) Inline config (MERGEN_CONFIG_CONTENT)
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${key}/.well-known/mergen` })
        const response = await fetch(`${key}/.well-known/mergen`)
        if (!response.ok) {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json"
        result = merge(result, await load(JSON.stringify(remoteConfig), `${key}/.well-known/mergen`))
    ...
    if (Flag.MERGEN_CONFIG) {
      result = merge(result, await loadFile(Flag.MERGEN_CONFIG))
      log.debug("loaded custom config", { path: Flag.MERGEN_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.MERGEN_DISABLE_PROJECT_CONFIG) {
      for (const file of ["mergen.jsonc", "mergen.json"]) {
    ...
    const directories = [
      Global.Path.config,
      // Only scan project .mergen/ directories when project discovery is enabled
      ...(!Flag.MERGEN_DISABLE_PROJECT_CONFIG
        ...
        : []),
      // Always scan ~/.mergen/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".mergen"],
    ...
    // .mergen directory config overrides (project and global) config sources.
    if (Flag.MERGEN_CONFIG_DIR) {
      directories.push(Flag.MERGEN_CONFIG_DIR)
      log.debug("loading config from MERGEN_CONFIG_DIR", { path: Flag.MERGEN_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(directories)) {
      if (dir.endsWith(".mergen") || dir === Flag.MERGEN_CONFIG_DIR) {
        for (const file of ["mergen.jsonc", "mergen.json"]) {
    ...
    if (Flag.MERGEN_CONFIG_CONTENT) {
      result = merge(result, JSON.parse(Flag.MERGEN_CONFIG_CONTENT))
      log.debug("loaded custom config from MERGEN_CONFIG_CONTENT")
    }
    ...
    if (Flag.MERGEN_PERMISSION) {
      try {
        result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.MERGEN_PERMISSION))
      } catch {
        log.warn("MERGEN_PERMISSION contains invalid JSON, skipping")
      }
    }
    ...
    if (Flag.MERGEN_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.MERGEN_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@mergen-io/plugin": targetVersion,
    }
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@mergen-io/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@mergen-io/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@mergen-io/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const patterns = ["/.mergen/command/", "/.mergen/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const patterns = ["/.mergen/agent/", "/.mergen/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * getPluginName("oh-my-mergen@2.4.3") // "oh-my-mergen"
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-mergen", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-mergen@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: mergen.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for mergen serve and web commands"),
      command: z
      ...
      default_agent: z
        ...
        .optional()
        .describe(
          "Default agent to use when none is specified. Must be a primary agent. Falls back to 'mergen' if not set or if the specified agent is invalid.",
        ),
      username: z
        .string()
        .optional()
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "mergen.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "mergen.jsonc"))),
    )
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://raw.githubusercontent.com/TiaMEOWS/mergen/main/schema/config.json"
  ...
  export async function update(config: Info) {
    const filepath = path.join(Instance.directory, "mergen.json")
  ...
  export function globalConfigFile() {
    const candidates = ["mergen.jsonc", "mergen.json", "config.json"].map((file) =>
