...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for mergen.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal mergen sessions
...
mergen acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via mergen's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | mergen acp
```
...
ACP sessions map cleanly to mergen's internal session model:
