...
import type { AssistantMessage, Event, MergenClient, SessionMessageResponse } from "@mergen-io/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: MergenClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `mergen auth login` in the terminal",
        name: "Login with mergen",
        id: "mergen-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Mergen stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const mergenProvider = providers.find((p) => p.id === "mergen")
    if (mergenProvider) {
      if (mergenProvider.models["big-pickle"]) {
        return { providerID: "mergen", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(mergenProvider.models))
      if (best) {
        return {
