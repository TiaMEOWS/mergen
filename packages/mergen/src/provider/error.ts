...
export namespace ProviderError {
  ...
  // fires on OpenAI models proxied through GitHub Copilot. Mergen cannot
  ...
  function error(providerID: string, error: APICallError) {
    if (providerID.includes("github-copilot") && error.statusCode === 403) {
      return "Please reauthenticate with the copilot provider to ensure your credentials work properly with Mergen."
    }
    ...
    if (isContentFilter(m)) {
      return {
        ...
        message:
          ...
          "(it also applies to OpenAI models proxied through GitHub Copilot) and cannot be disabled by Mergen. " +
