...
import PROMPT_MERGEN from "./prompt/mergen.txt"
...
export namespace Agent {
  ...
  const state = Instance.state(async () => {
    ...
    const result: Record<string, Info> = {
      mergen: {
        name: "mergen",
        description: "The default Mergen agent. AI-powered offensive security with full tool access.",
  ...
  export async function list() {
    ...
    return pipe(
      await state(),
      values(),
      sortBy([(x) => (cfg.default_agent ? x.name === cfg.default_agent : x.name === "mergen"), "desc"]),
    )
  }
