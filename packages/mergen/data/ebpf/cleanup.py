...
Mergen patterns. Critical for operational security — always run before
...
Part of Mergen offensive security platform.
...
COMPILED_PATTERNS = [re.compile(p, re.IGNORECASE) for p in MERGEN_PATTERNS]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Mergen eBPF Cleanup Tool - Enumerate and remove Mergen eBPF programs from the system",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  sudo python3 cleanup.py                      # List all eBPF programs
  sudo python3 cleanup.py --remove              # Remove Mergen programs
  ...
  sudo python3 cleanup.py --all                 # Show all programs (not just Mergen)
...
Operational Security:
  ...
  any eBPF tracing artifacts left by Mergen agents.
        """,
    )
    parser.add_argument(
        "--remove",
        action="store_true",
        default=False,
        help="Remove eBPF programs matching Mergen patterns",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Show all eBPF programs, not just Mergen ones",
    )
    parser.add_argument(
        "--json-output",
        action="store_true",
...
def is_mergen_program(name):
    """Check if a program name matches any Mergen pattern."""
    if not name:
        return False
...
def enumerate_via_bpftool():
    ...
    for prog in progs:
        ...
        entry["is_mergen"] = is_mergen_program(entry["name"])
...
def parse_bpftool_text_output(text):
    ...
    for line in text.strip().split("\n"):
        ...
        if header_match:
            ...
            current["is_mergen"] = is_mergen_program(current["name"])
            continue
        ...
        for entry in bpf_path.rglob("*"):
            ...
            if entry.is_file() or entry.is_symlink():
                ...
                programs.append({
                    ...
                    "pids": [],
                    "source": "bpffs",
                    "is_mergen": is_mergen_program(name),
...
def print_text_output(programs, methods_used, show_all=False, removal_results=None):
    ...
    cs_programs = [p for p in programs if p["is_mergen"]]
    other_programs = [p for p in programs if not p["is_mergen"]]

    print("=" * 90)
    print(f"{'Mergen eBPF Cleanup Tool':^90}")
    print("=" * 90)
    print()
    ...
    print(f"  Mergen programs found: {len(cs_programs)}")
    print(f"  Other programs found: {len(other_programs)}")
    print()

    if cs_programs:
        print("-" * 90)
        print("  MERGEN PROGRAMS (flagged for removal)")
        print("-" * 90)
    ...
    if not cs_programs and not removal_results:
        print("  [OK] No Mergen eBPF programs detected on this system.")
        print()
...
def main():
    ...
    if args.remove and cs_programs:
        if args.dry_run:
            if not args.json_output:
                print(f"[*] DRY RUN: Would remove {len(cs_programs)} Mergen program(s):")
                for p in cs_programs:
            ...
            if not args.force and not args.json_output:
                print(f"[!] About to remove {len(cs_programs)} Mergen eBPF program(s).")
    ...
    if args.json_output:
        output = {
            "tool": "mergen-ebpf-cleanup",
            "methods_used": methods_used,
            "total_programs": len(programs),
            "mergen_programs": len(cs_programs),
