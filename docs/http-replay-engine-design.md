...
Only **target-directed attack traffic** funnels through the engine. Mergen's
...
(b) the AI model, (c) Mergen / the proxy agents themselves. Concurrency is
...
| **Agent/LLM layer** (orchestration) | AI model + Mergen | **Bounded concurrency pool** (how many testers / ingest items run at once) + provider rate-limit awareness + **backpressure** (throttle new dispatch when the LLM slows)                                         |
...
- **Slowing Mergen** → backpressure + global budget; concurrency never
