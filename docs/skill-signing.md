...
Mergen uses Ed25519 digital signatures to guarantee the integrity and authenticity of official skills. This document covers the full signing architecture, verification flow, and maintainer operations.

## Architecture Overview

```
┌─────────────────────────────────────┐
│  Mergen Maintainer (offline)   │
│                                     │
...
| ----------- | ---------------------------------- | ------------------------------------------------------ | ---------------- |
| Public Key  | Base64-encoded raw 32-byte Ed25519 | `packages/mergen/src/skill/signing.ts` (embedded) | Yes              |
...
- The `MERGEN_SKILL_PRIVATE_KEY` environment variable can be used instead of the file
...
```bash
bun run packages/mergen/script/sign-skills.ts --generate
```
...
3. Embed the public key in `packages/mergen/src/skill/signing.ts`
4. Add `.skill-signing-key` to `.gitignore` (if not already present)
5. Sign all skills in `.mergen/skill/`

### Re-Sign All Skills (After Content Changes)

```bash
bun run packages/mergen/script/sign-skills.ts
```

Reads the private key from `.skill-signing-key` (or `MERGEN_SKILL_PRIVATE_KEY` env var) and signs all skills.

### Sign with Environment Variable

```bash
MERGEN_SKILL_PRIVATE_KEY="<base64-pkcs8-key>" bun run packages/mergen/script/sign-skills.ts
```
...
```bash
mergen skill verify              # verify all skills
mergen skill verify wstg-inpv-05 # verify a specific skill
```
...
| Malicious community skill pretending to be official  | No valid signature for `signed_by: mergen-official` → "tampered" → blocked |
...
- **The private key is the authority** — only the Mergen maintainer team can sign skills as official
...
git add packages/mergen/src/skill/signing.ts .mergen/skill/
...
| File                                              | Purpose                                                      |
| ------------------------------------------------- | ------------------------------------------------------------ |
| `packages/mergen/src/skill/signing.ts`       | Verification engine + embedded public key                    |
| `packages/mergen/src/skill/skill.ts`         | Calls `verify()` on skill load, blocks tampered              |
| `packages/mergen/script/sign-skills.ts`      | Maintainer tool: generate keypair + sign all skills          |
| `.skill-signing-key`                              | Private key (gitignored)                                     |
| `.mergen/skill/*/SKILL.md`                   | Signed skills with sha256/signature/signed_by in frontmatter |
| `packages/mergen/src/cli/cmd/skill.ts`       | CLI `mergen skill verify` command                       |
| `packages/mergen/src/server/routes/skill.ts` | REST `POST /skill/:name/verify` endpoint                     |
