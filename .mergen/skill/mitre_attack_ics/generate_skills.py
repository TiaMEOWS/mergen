...
# ICS platform mapping — ICS has no standard platform field, defaults to ICS
PLATFORM_MAP = {
    'None': 'ics',
    'Windows': 'windows',
    'Linux': 'linux',
    ...
    'Field Controller/RTU/PLC/IED': 'plc-rtu',
    'Input/Output Server': 'io-server',
    'Safety Instrumented System/Protection Relay': 'sis',
}

# ICS tactic to SKILL_GUIDE category mapping
TACTIC_CATEGORIES = {
    ...
    'impair-process-control': 'business-logic',
    'inhibit-response-function': 'business-logic',
}
...
def generate_skill_md(tech, tactic_info, mitigations, detections, is_sub, dir_name, children_map):
    ...
    # ICS platforms are mostly 'None' — replace with meaningful value
    if platforms == ['None'] or not platforms:
        platforms = ['ICS']
    ...
    tech_stack = ['ics']
    for p in platforms:
        mapped = PLATFORM_MAP.get(p, p.lower().replace(' ', '-'))
        if mapped not in tech_stack and mapped != 'ics':
            tech_stack.append(mapped)

    tags = ['mitre-attack', 'ics', ext_id.lower()]
    ...
    cwe_map = {
        ...
        'impact': ['CWE-400'],
        'impair-process-control': ['CWE-400'],
        'inhibit-response-function': ['CWE-400'],
    }
...
all_tactics:
{chr(10).join(f'  - {t}' for t in all_tactics)}
platforms:
...
---'''
    ...
    check_items = []
    check_items.append(f'- [ ] Identify if {name} technique is applicable to target ICS environment')
    check_items.append(f'- [ ] Check ICS/SCADA systems for indicators of {name}')
    if mitigations:
    ...
    severity = 'High' if any(k in (name + desc).lower() for k in ['credential', 'exploit', 'injection', 'hijack', 'rootkit', 'privilege', 'damage', 'destruction', 'denial', 'manipulat']) else 'Medium' if any(k in (name + desc).lower() for k in ['discovery', 'obfuscat', 'proxy', 'tunnel', 'monitor']) else 'Low'
    ...
    refs.append(f'- [MITRE ATT&CK ICS - {ext_id}]({url})')
    refs_section = '\n'.join(refs[:15])
...
Determine if the target ICS/SCADA environment is susceptible to {name} by examining operational technology systems and network architecture.

### Assess Existing Defenses

Review whether mitigations for {ext_id} are in place. If defenses are absent or misconfigured, this technique may be exploitable in the ICS environment.
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3 generate_skills.py /path/to/ics-attack.json')
        sys.exit(1)
    ...
    tactics = [o for o in data['objects'] if o['type'] == 'x-mitre-tactic' and not o.get('revoked', False) and not o.get('x_mitre_deprecated', False)]
    tactic_info = {}
    for t in tactics:
