#!/usr/bin/env python3
"""
MITRE ATT&CK Mobile SKILL.md Generator for Mergen
Reads STIX 2.1 JSON and generates skill files organized by tactic.

Follows SKILL_GUIDE.md conventions.

Usage:
    python3 generate_skills.py /path/to/mobile-attack.json
...
# Map ATT&CK Mobile platforms to tech_stack values
PLATFORM_MAP = {
    'Android': 'android',
    'iOS': 'ios',
}

# Mobile tactic to SKILL_GUIDE category mapping
TACTIC_CATEGORIES = {
    ...
    'remote-service-effects': 'configuration',
}
...
def generate_skill_md(tech, tactic_info, mitigations, detections, is_sub, dir_name, children_map):
    ...
    tags = ['mitre-attack', 'mobile', ext_id.lower()]
    ...
    tech_stack_yaml = '\n'.join(f'  - {t}' for t in tech_stack) if tech_stack else '  - mobile'
    ...
    cwe_map = {
        ...
        'impact': ['CWE-400'],
        'network-effects': ['CWE-300'],
        'remote-service-effects': ['CWE-284'],
    }
...
platforms:
{chr(10).join(f'  - {p}' for p in platforms) if platforms else '  - Mobile'}
mitre_url: "{url}"
tech_stack:
{tech_stack_yaml}
cwe_ids:
...
---'''
    ...
    check_items = []
    check_items.append(f'- [ ] Identify if {name} technique is applicable to target mobile environment')
    for p in platforms[:3]:
        check_items.append(f'- [ ] Check {p} devices for indicators of {name}')
    if mitigations:
    ...
    refs.append(f'- [MITRE ATT&CK Mobile - {ext_id}]({url})')
    refs_section = '\n'.join(refs[:15])
...
**Platforms:** {', '.join(platforms) if platforms else 'Mobile'}
...
Determine if the target mobile environment is susceptible to {name} by examining the target platforms ({', '.join(platforms[:3]) if platforms else 'Android, iOS'}).
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3 generate_skills.py /path/to/mobile-attack.json')
        sys.exit(1)
