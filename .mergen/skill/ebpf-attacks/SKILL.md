...
| BPF integrity | `ebpf bpf_integrity --baseline --duration 300` | Verify Mergen hook integrity, detect unauthorized BPF program loads |
...
**BPF integrity verification** takes a baseline of loaded BPF programs via `bpftool` and periodically verifies no Mergen programs have been detached or tampered with. Also monitors `bpf()` syscall for unauthorized program loads.
...
```bash
# List all Mergen eBPF programs on the system
ebpf cleanup

# Remove all Mergen eBPF programs
