#!/usr/bin/env python3
"""
MITRE ATT&CK Enterprise SKILL.md Generator for Mergen with Atomic Red Team Integration
Reads STIX 2.1 JSON and Atomic Red Team YAML to generate actionable skill files.

NEW: Integrates Atomic Red Team tests into "How to Test" section for actionable pentesting guidance.

Usage:
    python3 generate_skills_with_art.py /path/to/enterprise-attack.json /path/to/atomic-red-team/atomics/
...
def load_atomic_tests(ext_id, atomics_dir):
    """Load Atomic Red Team tests for a technique from YAML."""
    if not atomics_dir:
        return []

    yaml_path = atomics_dir / ext_id / f'{ext_id}.yaml'
...
def format_atomic_tests(atomic_tests, platforms):
    """Format Atomic Red Team tests into markdown section."""
    if not atomic_tests:
        return None

    parts = []
    for idx, test in enumerate(atomic_tests[:5], 1):  # Limit to 5 tests
        ...
        # Skip if not supported on any target platform
        if platforms and not any(p.lower() in [s.lower() for s in supported] for p in platforms):
            continue

        parts.append(f'### Atomic Test {idx}: {name}')
        parts.append('')
        if desc:
            parts.append(desc)
            parts.append('')
        parts.append(f'**Supported Platforms:** {", ".join(supported)}')
        if elevation:
            parts.append('**Elevation Required:** Yes')
        parts.append('')
        ...
        if dependencies:
            ...
            for dep in dependencies:
                dep_desc = dep.get('description', '').strip()
                if dep_desc:
...
def generate_skill_md(tech, tactic_info, mitigations, detections, is_sub, dir_name, children_map, atomic_tests):
    """Generate SKILL.md content with Atomic Red Team tests."""
    ext_id = tech['external_references'][0]['external_id']
    name = tech['name']
...
---'''
    ...
    # HOW TO TEST — Enhanced with Atomic Red Team
    atomic_section = format_atomic_tests(atomic_tests, platforms)

    if atomic_section:
        test_section = f'''### Atomic Red Team Tests
...
1. **Identify Attack Surface**: Determine if the target environment is susceptible to {name} by examining the target platforms ({', '.join(platforms[:3]) if platforms else 'any'}).

2. **Assess Existing Defenses**: Review whether mitigations for {ext_id} are in place. If defenses are absent or misconfigured, this technique may be exploitable.

3. **Execute Test**: Use tools and methods described in the MITRE ATT&CK page and external references below.'''
    else:
        test_section = f'''### Manual Testing
...
> **Note**: No Atomic Red Team tests available for this technique. See [Atomic Red Team GitHub](https://github.com/redcanaryco/atomic-red-team) for updates.'''
    ...
    refs.append(f'- [Atomic Red Team - {ext_id}](https://github.com/redcanaryco/atomic-red-team/tree/master/atomics/{ext_id})')
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3 generate_skills_with_art.py /path/to/enterprise-attack.json [/path/to/atomic-red-team/atomics/]')
        sys.exit(1)

    stix_path = sys.argv[1]
    atomics_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else None
    output_dir = Path(__file__).parent

    if atomics_dir and not atomics_dir.exists():
        print(f'Warning: Atomics directory not found: {atomics_dir}')
        print('Continuing without Atomic Red Team tests...')
        atomics_dir = None
    ...
    if atomics_dir:
        print(f'Using Atomic Red Team tests from {atomics_dir}')
    ...
    for tech in sorted(techniques, key=lambda x: x['external_references'][0]['external_id']):
        ...
        atomic_tests = load_atomic_tests(ext_id, atomics_dir) if atomics_dir else []
        if atomic_tests:
            with_atomics += 1

        # Generate SKILL.md with Atomic Red Team tests
        content = generate_skill_md(tech, tactic_info, mitigations, detections, is_sub, tech_dir_name, children_map, atomic_tests)
    ...
    print(f'Skills with Atomic Red Team tests: {with_atomics}/{created}')
    print('\nPer tactic:')
    for short, info in sorted(tactic_info.items(), key=lambda x: x[1]['id']):
