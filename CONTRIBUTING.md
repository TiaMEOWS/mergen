...
Mergen is an AI-powered offensive security platform. We welcome contributions that make it a more powerful, reliable, and comprehensive security testing tool.
...
- [`help wanted`](https://github.com/TiaMEOWS/mergen/issues?q=is%3Aissue+state%3Aopen+label%3Ahelp-wanted)
- [`good first issue`](https://github.com/TiaMEOWS/mergen/issues?q=is%3Aissue+state%3Aopen+label%3A%22good+first+issue%22)
- [`bug`](https://github.com/TiaMEOWS/mergen/issues?q=is%3Aissue+state%3Aopen+label%3Abug)
- [`security-tool`](https://github.com/TiaMEOWS/mergen/issues?q=is%3Aissue+state%3Aopen+label%3Asecurity-tool)
...
```bash
bun run --cwd packages/mergen build --single
./packages/mergen/dist/mergen-<platform>/bin/mergen
```
...
| Package                | Description                                       |
| ---------------------- | ------------------------------------------------- |
| `packages/mergen` | Core CLI — agents, tools, session, provider logic |
| `packages/app`         | Web UI components (SolidJS)                       |
| `packages/plugin`      | Plugin SDK (`@mergen-io/plugin`)             |
| `knowledge/`           | Security knowledge base (WSTG test cases)         |
| `.mergen/skill/`  | Security skills (methodology guides)              |
...
Skills are markdown files that provide domain knowledge to agents. Create a new directory under `.mergen/skill/`:

```
.mergen/skill/your-skill-name/
  SKILL.md    # Methodology, checklists, tool commands
```

Reference it in an agent's `skills` array in `packages/mergen/src/agent/agent.ts`.
...
1. Create a system prompt: `packages/mergen/src/agent/prompt/your-agent.txt`
2. Register in `packages/mergen/src/agent/agent.ts` as a native agent
...
```bash
bun dev                    # Start Mergen TUI (development)
...
```bash
bun run --inspect=ws://localhost:6499/ --cwd packages/mergen ./src/index.ts serve --port 4096
```
...
Mergen is designed for **authorized security testing only**. All contributions must:
...
For new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach, and why it belongs in Mergen. Wait for core team approval before opening a PR.

## Community

- **Discord:** [Join the community](https://discord.gg/snunAaHf6U)
- **X:** [@mergen](https://x.com/mergen)
- **Website:** [mergen.dev](https://mergen.dev)
